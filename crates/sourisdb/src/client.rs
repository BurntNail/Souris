...
use crate::store::{Store, StoreSerError};
use crate::values::{Value, ValueSerError, ValueTy};
...
impl Display for ClientError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::IncorrectType {ex, found} => write!(f, "Expected to find value of type {ex:?}, found {found:?}"),
            Self::ServerNotHealthy(sc) => write!(f, "Tried to get server health check, got status code: {sc:?}"),
...
impl SourisClient {
    pub fn new (path: impl Display, port: u32) -> Result<Self, ClientError> {
        ...
        let rsp = agent.get(&format!("http://{path}:{port}/healthcheck")).call()?;
        if rsp.status() != StatusCode::OK {
            eprintln!("Server is not healthy.");
            return Err(ClientError::ServerNotHealthy(StatusCode::try_from(rsp.status())?));
        }
    ...
    pub fn get_all_dbs (&self) -> Result<Vec<String>, ClientError> {
        let rsp = self.agent.get(&format!("http://{}:{}/v1/get_all_dbs", self.path, self.port)).call()?;
    ...
    pub fn create_new_db (&self, overwrite_existing: bool, name: String) -> Result<bool, ClientError> {
        let rsp = self.agent.post(&format!("http://{}:{}/v1/add_db", self.path, self.port))
            .query("overwrite_existing", if overwrite_existing {"true"} else {"false"})
    ...
    pub fn get_store (&self, db_name: String) -> Result<Store, ClientError> {
        let rsp = self.agent.get(&format!("http://{}:{}/v1/get_db", self.path, self.port))
    ...
    pub fn add_db_with_contents (&self, overwrite_existing: bool, name: String, store: Store) -> Result<bool, ClientError> {
        let store = store.ser()?;
        let rsp = self.agent.get("http://{}:{}/v1/add_db_with_content")
            .query("overwrite_existing", if overwrite_existing {"true"} else {"false"})
            .query("name", name.as_str())
            .send_bytes(&store)?;
        Ok(match rsp.error_for_status()? {
    ...
    pub fn add_entry_to_db (&self, database_name: String, key: String, value: Value) -> Result<bool, ClientError> {
        let value = value.ser()?;
        let rsp = self.agent.put(&format!("http://{}:{}/v1/add_kv", self.path, self.port))
    ...
    pub fn remove_entry_from_db (&self, database_name: String, key: String) -> Result<(), ClientError> {
        let rsp = self.agent.post(&format!("http://{}:{}/v1/rm_key", self.path, self.port))
            .query("db", database_name.as_str())
            .query("key", key.as_str())
            .call()?;
        rsp.error_for_status()?;
    ...
    pub fn remove_db (&self, database_name: String) -> Result<(), ClientError> {
        let rsp = self.agent.post(&format!("http://{}:{}/v1/rm_db", self.path, self.port))
